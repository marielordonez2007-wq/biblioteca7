...
def pantalla_login():
    ...
    with tab_registro:
        with st.form("form_registro"):
            ...
            st.write("Preguntas de seguridad")
            ...
            pregunta3 = st.selectbox("Pregunta 3", opciones, key="reg_p3")
            respuesta3 = st.text_input("Respuesta 3", key="reg_r3")
            enviado = st.form_submit_button("Registrarme", type="primary")
        if enviado:
            if password != confirmar:
                st.error("Las contrasenas no coinciden.")
            elif "Seleccione una pregunta" in (pregunta1, pregunta2, pregunta3):
                st.error("Debe seleccionar las tres preguntas de seguridad.")
            else:
    ...
    with tab_recuperar:
        ...
        if st.session_state.recuperacion_usuario is None:
            with st.form("form_identificar_recuperacion"):
                nombre_usuario = st.text_input("Nombre de usuario", key="rec_usuario")
            ...
            if continuar:
                ...
                elif not usuario.get("pregunta_seguridad") or not usuario.get("pregunta_seguridad_2") or not usuario.get("pregunta_seguridad_3"):
                    st.error("Esta cuenta no tiene configuradas las tres preguntas de seguridad.")
                else:
            ...
            with st.form("form_verificar_recuperacion"):
                ...
                respuesta3 = st.text_input(usuario["pregunta_seguridad_3"], type="password", key="rec_r3")
                verificar = st.form_submit_button("Verificar respuestas", type="primary")
            if verificar:
                verificado, error = db.verificar_preguntas_seguridad(
                    usuario["usuario"], [respuesta1, respuesta2, respuesta3]
                )
                if error:
                    st.error(error)
                else:
                ...
                if cambiar:
                    if nueva != confirmar:
                        ...
                        ok, mensaje = db.recuperar_password(
                            usuario["usuario"], [respuesta1, respuesta2, respuesta3], nueva
                        )
                        if ok:
...
def pantalla_perfil(usuario):
    ...
    with st.expander("Actualizar preguntas de seguridad"):
        ...
        with st.form("form_actualizar_seguridad"):
            ...
            p3 = st.selectbox("Pregunta 3", opciones, key="perfil_p3")
            r3 = st.text_input("Respuesta 3", key="perfil_r3")
            guardar_seguridad = st.form_submit_button("Guardar preguntas")
        if guardar_seguridad:
            if "Seleccione una pregunta" in (p1, p2, p3):
                st.error("Debe seleccionar las tres preguntas.")
            else:
                ok, mensaje = db.actualizar_preguntas_seguridad(
                    usuario["codigo"], [(p1, r1), (p2, r2), (p3, r3)]
                )
                (st.success if ok else st.error)(mensaje)
