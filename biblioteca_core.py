...
class Biblioteca:
    ...
    def crear_tablas(self):
        ...
        self.con.execute(
            "UPDATE usuarios SET pregunta_seguridad_2 = ?, pregunta_seguridad_3 = ? "
            "WHERE trim(pregunta_seguridad_2) = ''",
            (PREGUNTAS_SEGURIDAD[1], PREGUNTAS_SEGURIDAD[2])
        )
        self.con.execute(
            "UPDATE usuarios SET respuesta_seguridad_2 = respuesta_seguridad, respuesta_seguridad_3 = respuesta_seguridad "
    ...
    def asegurar_administrador(self):
        fila = self.con.execute(
            "SELECT codigo, rol FROM usuarios WHERE lower(usuario) = 'admin' OR lower(codigo) = 'admin' LIMIT 1"
        ).fetchone()
        ...
        for usuario in USUARIOS_INICIALES:
            ...
            self.con.execute(
                ...
                (
                    ...
                    PREGUNTAS_SEGURIDAD[2], hash_password(respuestas[2])
                )
            )
    ...
    def crear_usuario(self, nombre, usuario_login, telefono, password, preguntas, respuestas):
        ...
        if not re.fullmatch(r"[a-zA-Z0-9_.-]{3,30}", usuario_login):
            return None, "El nombre de usuario debe tener entre 3 y 30 caracteres y solo usar letras, numeros, punto, guion o guion bajo."
        ...
        if len(preguntas) != 3 or len(respuestas) != 3:
            return None, "Debe configurar las tres preguntas de seguridad."
        if len(set(preguntas)) != 3:
            return None, "Debe seleccionar tres preguntas de seguridad diferentes."
        if any(not r.strip() for r in respuestas):
            return None, "Debe responder las tres preguntas de seguridad."
        ...
        try:
            self.con.execute(
                ...
                (codigo, usuario_login, nombre, telefono, hash_password(password),
                 ...
                 preguntas[2], hash_password(respuestas[2].strip().lower()))
            )
    ...
    def recuperar_password(self, usuario_login, respuestas):
        usuario = self.buscar_usuario_login(usuario_login)
        if usuario is None:
            return None, "No se encontro el nombre de usuario."
        if len(respuestas) != 3:
            return None, "Debe responder las tres preguntas de seguridad."
        ...
        almacenadas = [
            ...
            usuario.get("respuesta_seguridad_3", ""),
        ]
        if any(not x for x in almacenadas):
            return None, "Esta cuenta no tiene configuradas las tres preguntas de seguridad."

        for guardada, respuesta in zip(almacenadas, respuestas):
            if guardada != hash_password(respuesta.strip().lower()):
                return None, "Una o mas respuestas de seguridad no coinciden."
